...
终端不是普通扫描任务的唯一执行入口。扫描器仍应通过 Scanner Adapter 执行，以便得到结构化结果。
...
9. 终端命令和输出能保存并关联为证据。
10. 用户能记录 flag/loot。
11. 系统能生成 Markdown writeup 草稿。
12. 关闭并重启客户端后，数据可以恢复。
13. 外部工具缺失时，UI 和 API 返回可理解的诊断信息。
