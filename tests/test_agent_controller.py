...
def test_agent_controller_returns_legacy_bridge_for_active_task(tmp_path):
    settings = build_settings(tmp_path)
    session_service = SessionService.from_settings(settings)
    controller = AgentController.from_session_service(session_service)
    ...
    result = controller.handle(
        ControllerRequest(
            raw_input="Continue the bound task",
...
def test_build_controller_request_and_clear_command_preserve_session_binding(tmp_path):
    ...
    assert shell_state.active_session_mode == "normal"
