...
from app.writeup_service import WRITEUP_SECTIONS, WriteupService
...
def test_empty_session_writeup_has_required_skeleton(tmp_path):
    ...
    assert (tmp_path / ".red-code" / "projects" / session.project_id / "sessions" / session.id / "reports" / "writeup.md").is_file()
